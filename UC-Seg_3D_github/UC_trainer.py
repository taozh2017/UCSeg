...
from utils.losses import loss_diff1, loss_diff2, dice_loss

from module.denseCL_SSIM import Contrast_global_consistency
from utils.UMIS import infer, trans_infer, u_loss, pseudo_infer
from utils.utils import one_hot_encoder, val_coef
...
class UC_trainer(nn.Module):
    def __init__(self, args):
        ...
        self.best_performance_pesudo = 0.0
        self.args = args
        self.UC_seg = UC_Seg(args).cuda().train()
    ...
    def load_model_weights(self, model, pretrain):
    ...
    def train_(self, volume_batch, label_batch, iter_num, logger):
        ...
        label_batch = one_hot_encoder(label_batch.unsqueeze(1), n_classes=self.args.num_classes)
        ...
        if self.args.nms:
            unlabeled_pseudo_map = get_cut_mask(unlabeled_pseudo_map, nms=1)
        ...
        if self.args.nms:
            outputs_mask_1 = get_cut_mask(outputs_1, nms=1)
            outputs_mask_2 = get_cut_mask(outputs_2, nms=1)
        ...
        ssim_thr = 0.2 + 0.56 * (iter_num / self.args.max_iterations)
        ...
        if iter_num < 6000:
            ...
            img_mask, loss_mask = generate_mask_3D(labeled_volume_batch)
            ...
            # mix contrast loss
            if self.args.nms:
                outputs_mixed_mask_1 = get_cut_mask(outputs_mixed_1, nms=1)
                outputs_mixed_mask_2 = get_cut_mask(outputs_mixed_2, nms=1)
    ...
    def save(self, snapshot_path, iter_num, mode=None):
        if mode == "best":
            save_cur = os.path.join(snapshot_path, 'best_' + str(iter_num) +'.pth')
        else:
            save_cur = os.path.join(snapshot_path, 'pseudo_' + str(iter_num) + '.pth')

        torch.save(self.UC_seg.state_dict(), save_cur)

    
    def test(self,):
        self.UC_seg.eval()
        dice, hd95 = test_calculate_metric(self.args, self.UC_seg, val=True)
        self.UC_seg.train()
        return dice, hd95
